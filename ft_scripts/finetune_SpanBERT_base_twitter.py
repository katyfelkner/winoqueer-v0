...
# load news dataset
df = pickle.load(open('all_tweets_en.pkl', 'rb'))
# tweet normalization
df['normalized'] = df['text'].apply(normalizeTweet)
dataset = Dataset.from_pandas(df)
dataset = dataset.remove_columns(['tweetid', 'userid', 'username', 'date', 'lang', 'location',
       ...
       'rp_text', 'tweet_type', 'ref_date', 'rt_userid', 'rt_username',
                                  'qt_userid', 'qt_username', 'rp_userid', 'rp_username', 'hashtag', 'text'])
datasets = dataset.train_test_split(test_size=0.1)

# define tok function
def tokenize_function(examples):
    return tokenizer(examples["normalized"])
...
pretrained_model = "./pretrained/spanbert-base-cased"
...
training_args = TrainingArguments(
    f"{pretrained_model}-finetuned-lbgtq-twitter",
...
model.save_pretrained(f"{pretrained_model}-finetuned-lbgtq-twitter")
